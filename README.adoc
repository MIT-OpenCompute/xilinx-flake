...
==== Fixing app launcher / drun entries

If you enabled desktop shortcuts in the installer (`CreateDesktopShortcuts` /
`CreateProgramGroupShortcuts` in `install_config.txt`), Xilinx's own installer
drops `.desktop` files in `~/.local/share/applications/` and `~/Desktop/`
whose `Exec=` points straight at the raw, unwrapped binary under
`$INSTALL_DIR`. That binary isn't runnable outside the FHS environment this
flake builds, so launching it from an app launcher (drun, GNOME/KDE menus,
etc.) silently fails - no error is shown, nothing happens. Run this after
installing or re-installing to repoint those shortcuts at the Nix-wrapped
executables instead:

....
nix run github:MIT-OpenCompute/xilinx-flake#fix-desktop-entries
....
...
This corresponds to the **"Launch runs on remote hosts"** option in Vivado's
_Run Settings_ dialog (_Flow → Run Settings_ or right-click a run in the
_Design Runs_ panel).

. Generate an SSH key pair on the client workstation if you do not already
  have one:
+
....
...
. Add the **public** key (`~/.ssh/id_vivado_remote.pub`) to
  `services.vivadoServer.remoteRuns.authorizedKeys` in the server's NixOS
  configuration and rebuild.

. In Vivado, open _Tools → Options → Remote Hosts_ (or click
  *Configure Hosts* in the Launch Runs dialog) and add a new host:
+
[cols="1,2"]
|===
|Field |Value
...
|Value of `services.vivadoServer.remoteRuns.workDir`
(default: `/var/lib/vivado-remote`)
|===

. Select *Launch runs on remote hosts* in the Run Settings dialog, choose the
  host you configured, and click *OK*.  Vivado will SSH into the server and
  run synthesis/implementation in batch mode there; results are copied back
  automatically.

NOTE: Vivado invokes the remote command as `bash --login -c "vivado -mode
...
Edit the generated `flake.nix` to set your `top` module name, FPGA `part`
...
Both commands rsync whatever is currently in the configured `rtlDirs` on
...
|Base directory for per-project build artifacts.  ichika creates a
sub-directory per top module name here.

|`hdlBuild.licenseFile`
|`null`
|`XILINXD_LICENSE_FILE` injected into batch Vivado processes.  Set to
`@localhost` when `licenseServer.enable = true`.  Falls back to the
`remoteRuns.licenseFile` value if that service is also enabled.

|`hdlBuild.openFirewall`
|`false`
|Opens SSH port 22 in the firewall.  Shared with `remoteRuns.openFirewall`
— either flag being true opens the port.
|===
